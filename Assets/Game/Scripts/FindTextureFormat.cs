...
public class FindTextureFormat : MonoBehaviour
{
    ...
    private void Start()
    {
        ...
        // Logger.Instance.ShowDebugLog("error material: " + material.parent.name);
    }

    List<Shader> GetAllObjectsOnlyInScene()
    {
        Logger.Instance.ShowDebugLog("Checking all shaders");
        ...
        foreach (var go in FindObjectsOfTypeAll(typeof(Shader)) as Shader[])
        {
            ...
            {
                Logger.Instance.ShowDebugLog("SHADER NOT SUPPORTED: " + go.name);
    ...
    {
        Logger.Instance.ShowDebugLog("Checking all TextureFormats");
        ...
        foreach (var go in FindObjectsOfTypeAll(typeof(Texture2D)) as Texture2D[])
        {
            ...
            {
                Logger.Instance.ShowDebugLog("TextureFormat NOT SUPPORTED: " + go.name + ", supported? " + b);
    ...
    {
        Logger.Instance.ShowDebugLog("Checking all materials");
        ...
        foreach (var go in FindObjectsOfTypeAll(typeof(Material)) as Material[])
        {
            ...
            {
                // Logger.Instance.ShowDebugLog("material shader NOT SUPPORTED: " + go.name + ", go.mainTexture.name: " + go.mainTexture.name);
                matsList.Add(go);
            }
        }
    ...
    {
        Logger.Instance.ShowDebugLog("Checking all render textures");
        ...
        {
            Logger.Instance.ShowDebugLog("render texture: " + obj.name);
        ...
        {
            Logger.Instance.ShowDebugLog("resource render texture: " + obj.name);
            if (obj.graphicsFormat == GraphicsFormat.R32G32B32A32_SFloat ||
                obj.format == RenderTextureFormat.ARGBFloat)
            {
                Logger.Instance.ShowDebugLog("BAD render texture: " + obj.name);
                list.Add(obj);
            }
        }
