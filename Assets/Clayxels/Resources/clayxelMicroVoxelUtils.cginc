...
float _splatPixelWidthThreshold;
...
uniform StructuredBuffer<float4x4> instancesObjectMatrix;
uniform StructuredBuffer<float4x4> instancesObjectMatrixInv;
uniform StructuredBuffer<int> instancesId;
//
...
void clayxels_microVoxels_vert(float invertFlip, uint chunkId, uint vertexId, float3 positionOS, out float3 vertexPos, out float3 boundsCenter, out float3 boundsSize, out float outBoxFlipped){
    vertexPos = 0;
    boundsCenter = 0;
    boundsSize = 0;
    outBoxFlipped = 0;
...
inline void traverseMip3(float lodFactor, float sizeMult, float3 normalOrient, float roughOffset,
...
inline void intersectSplat(float lodFactor, float pixelRandom, float3 voxelFragPos, float3 viewDirectionWS, int3 cellCoord, uint neighbourCellCoordId, int chunkOffsetIdMip2, int chunkOffsetMip3, float testerBoundsMip2, uint cellCoordIdMip2, float3 cellCornerMip2, float cellSizeMip3, float halfCellSizeMip3, float cellSizeMip3Splat, inout float depthTest, inout float3 hitNormal, inout float hitNormalOffset, inout int hitMip3ColorData, inout float hitColorRough, inout float3 hitDepthPoint){
    ...
    traverseMip3(lodFactor, roughSize, normalOffset, roughOffset,
...
bool clayxels_microVoxelsMip3SplatSeed_frag(float4 screenPos, float4x4 modelMatrix, float4x4 objectMatrixInv, uint chunkId, float3 positionWS, float3 viewDirectionWS, float3 boundsCenter, float3 boundsSize, float3 localCamPos, float lodFactor, out float3 hitNormal, out float3 hitColor, out float3 hitDepthPoint, out int hitClayObjId, out float4 hitGridData){
    hitNormal = 0;
    hitColor = 0.0;
    ...
    float march = 1.0;
    uint traverseSteps = lerp(150, 200, lodFactor) * marchReducer;
    for(uint traverseMip2It = 0; traverseMip2It < traverseSteps; ++traverseMip2It){
        ...
        // descend to mip3
        traverseMip3(lodFactor, 1.0, normalOffset, roughOffset,
            voxelFragPos, viewDirectionWS, cellSizeMip3, cellSizeMip3Splat, cellCenter, mipLevel, testerBounds, testerBoundsMip3, mip2Pointer, chunkOffsetMip3, cellCoordId, march, hit, hitDepthPoint, hitNormalTmp, hitNormalOffset, splatCenter, hitMip3ColorData);
    ...
    uint numNeighbours = 27 * hit * lodFactor;// has to be scaled down with distance to optimize perf
    ...
    numNeighbours = 12 * hit * lodFactor;// has to be scaled down with distance to optimize perf
...
void clayxelMicrovoxelVert(uint instanceId, uint vertexId, float3 vertexPos, out float3 outVertexPos, out float3 outBoundsCenter, out float3 outBoundsSize, out float4 outScreenPos){
    float4x4 objectMatrixInv = instancesObjectMatrixInv[instanceId];
    float4x4 objectMatrix = instancesObjectMatrix[instanceId];
    ...
    int outBoxFlipped = 0;
    clayxels_microVoxels_vert(invertFlip, chunkId, vertexId, vertexPos, outVertexPos, boundsCenter, boundsSize, outBoxFlipped);
    
    outVertexPos = mul(objectMatrix, float4(outVertexPos,1)).xyz - renderBoundsCenter;
...
inline void clayxelMicrovoxelFragForward(int instanceId, float3 vertexPos, uint vertexId, float4 screenPos, float3 boundsCenter, float3 viewDir, inout float3 outSplatPos, inout float3 outColor, inout float3 outNormal, inout float3 outEmission, inout float outDepth, inout float outAlpha){
    float4 bufferTexCoord = float4(screenPos.xy / screenPos.w, 0, 0);

    float4 buffer0 = tex2Dlod(microVoxRenderTex0, bufferTexCoord);
    
    uint containerIdAtPixel = rgbToInt(buffer0.xyz);

    outAlpha = 1;

    containerId = instancesId[instanceId];
    ...
    float depth = rgbToFloat(depthRGB.xyz);
    ...
    #ifdef CLAYXELS_URP
        // reconstructing world space from gridData for now because reversing z-depth is giving troubles
        // outDepth = depth;
        // float4 posCS = float4(bufferTexCoord.xy * 2.0 - 1.0, depth, 1.0);
        // float4 posInvProj = mul(UNITY_MATRIX_I_VP, posCS);
        // float3 splatPos = posInvProj.xyz / posInvProj.w;
        // outSplatPos = splatPos;

        float4 backdropGridData = tex2Dlod(microVoxRenderTex4, bufferTexCoord);
        float offset = ((backdropGridData.w * 2.0) - 1.0) * cellSizeMip3;
        ...
        vertexPos = vertexPos + renderBoundsCenter;
        
        // project splatPos at pixel to avoid having shadows at pixel resolution
        float3 viewDirectionPersp = normalize(_WorldSpaceCameraPos.xyz - vertexPos);
        float3 viewDirectionIso = UNITY_MATRIX_V[2].xyz;
        float3 viewDirectionWS = lerp(viewDirectionPersp, viewDirectionIso, perspIso);
        float3 perPixelSplatPos = projectRayOnPlane(vertexPos, viewDirectionWS, splatPos, splatNormalOriented);
    ...
    float emission = emissionMult * _emissionPower;
        ...
        float3 sssDelta = outSplatPos - sssCenter;
...
inline void clayxelMicrovoxelFragShadow(int instanceId, float3 viewDir, float3 boundsCenter, float3 boundsSize, uint vertexId, float3 vertexPos, float4 screenPos, inout float outDepth, inout float outAlpha){
    ...
    outAlpha = 1;
    bool hit = clayxels_microVoxelsMip3SplatSeed_frag(screenPos, objectMatrix, objectMatrixInv, chunkId, positionWS, viewDirectionWS, boundsCenter, boundsSize, localCamPos, lodFactor, hitNormal, hitColor, hitDepthPoint, hitClayObjId, hitGridData);
...
void clayxelMicrovoxelFrag(float3 viewDir, float instanceId, float3 boundsCenter, float3 boundsSize, int vertexId, float3 vertexPos, float4 screenPos, out float3 outSplatPos, out float3 outColor, out float3 outNormal, out float3 outEmission, out float outDepth, out float outAlpha){
    outSplatPos = 0;
    outColor = 0;
    outNormal = 0;
    outEmission = 0;
    ...
    outAlpha = 0;
        ...
        clayxelMicrovoxelFragForward(round(instanceId), vertexPos, vertexId, screenPos, boundsCenter, viewDir, outSplatPos, outColor, outNormal, outEmission, outDepth, outAlpha);
...
MVPassVertOut MicroVoxelPassVert(MVPassVertIn input)
{
    MVPassVertOut output = (MVPassVertOut)0;

    int containerId = instancesId[input.instanceID];

    int chunkId = float3(input.vertexID / 8, 0, 0);
    output.instanceData.x = chunkId;

    float4x4 objectMatrixInv = instancesObjectMatrixInv[input.instanceID];
    float4x4 objectMatrix = instancesObjectMatrix[input.instanceID];
    ...
    clayxels_microVoxels_vert(invertFlip, chunkId, input.vertexID, input.positionOS.xyz, vertexPos, boundsCenter, boundsSize, outBoxFlipped);
    ...
    float3 cellSizeWS = modelPos + (UNITY_MATRIX_V._m00_m10_m20 * cellSize);
    ...
    float LOD = saturate(cellSizePixels * rcp(_splatPixelWidthThreshold));

    output.instanceData = float3(chunkId, input.instanceID, LOD);

    #if defined(CLAYXELS_URP)
        float3 v = mul(mul(unity_WorldToObject, objectMatrix), float4(vertexPos,1)).xyz;
    #else
        float3 v = mul(objectMatrix, float4(vertexPos, 1)).xyz;// - renderBoundsCenter;
    #endif 
...
MVPassFragOut MicroVoxelPassFrag(MVPassVertOut input, 
    ...
    int chunkId = round(input.instanceData.x);
    int instanceId = round(input.instanceData.y);
    float lod = input.instanceData.z;
    ...
    bool hit = false;
    hit = clayxels_microVoxelsMip3SplatSeed_frag(input.screenPos, input.modelMatrix, instancesObjectMatrixInv[instanceId], chunkId, voxelPos, voxelSpaceViewDir, input.boundsCenter, input.boundsSize, input.localCamPos, lod, hitNormal, hitColor, hitDepthPoint, hitClayObjId, hitGridData);
    ...
    int containerChunkId = ((containerId * maxChunks) + chunkId) + 1;
    float3 containerIdRGB = unpackRgb(containerChunkId);
    float3 clayObjIdRGB = unpackRgb(hitClayObjId);

    float3 depthPacked = floatToRgb(outDepth);

    outBuffers.buffer0 = float4(containerIdRGB, clayObjIdRGB.x);
    outBuffers.buffer1 = float4(hitNormal, clayObjIdRGB.y);
    outBuffers.buffer2 = float4(hitColor, clayObjIdRGB.z);
    outBuffers.buffer3 = float4(depthPacked, 1);
    outBuffers.buffer4 = hitGridData;
